...
// GetComponentTypes reads the declared types of one component from the
// active graph. The graph is the single data source of the procedure.
func (service *graphService) GetComponentTypes(
	ctx context.Context,
	request *connect.Request[goconductv1.GetComponentTypesRequest],
) (*connect.Response[goconductv1.GetComponentTypesResponse], error) {
	result, err := query.ComponentTypes(
		service.source.currentGraph(),
		request.Msg.GetComponentId(),
	)
	if err != nil {
		return nil, connecterror.From(ctx, err)
	}
	return connect.NewResponse(&goconductv1.GetComponentTypesResponse{
		Revision:    result.Analysis.Revision,
		ComponentId: result.Component,
		Types:       typeDeclarationsToProto(result.Types),
		Incoming:    incomingTypeRelationsToProto(result.Incoming),
	}), nil
}
