...
func compileComponentPathRule(
	...
	rule := componentPathRule{role: role, category: category, segments: segments}
	if !placeholders.contains("component") && role != componentRoleApplication {
		rule.exact = true
		rule.name = pathSegments[len(pathSegments)-1]
	}
	return rule, nil
}
...
func (classifier componentClassifier) classify(relativePath string) (componentDescriptor, bool) {
	...
	if strings.HasSuffix(normalizedPath, ".go") {
		separator := strings.LastIndexByte(normalizedPath, '/')
		if separator < 0 {
			normalizedPath = ""
		} else {
...
func (rule componentPathRule) matchPath(pathSegments []string) (map[string]string, bool) {
	...
	if rule.exact && len(pathSegments) != len(rule.segments) {
		return nil, false
	}
