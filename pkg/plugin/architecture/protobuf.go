...
func typeDeclarationsToProto(declarations []TypeDeclaration) []*goconductv1.TypeDeclaration {
	result := make([]*goconductv1.TypeDeclaration, 0, len(declarations))
	for _, declaration := range declarations {
		result = append(result, &goconductv1.TypeDeclaration{
			Id:         declaration.Identifier,
			Name:       declaration.Name,
			Package:    declaration.Package,
			Component:  declaration.Component,
			Path:       declaration.Path,
			Line:       countToProto(declaration.Line),
			Kind:       string(declaration.Kind),
			Exported:   declaration.Exported,
			Test:       declaration.Test,
			Underlying: declaration.Underlying,
			Fields:     typeFieldsToProto(declaration.Fields),
			Methods:    typeMethodsToProto(declaration.Methods),
			Embeds:     typeReferencesToProto(declaration.Embeds),
			Implements: typeReferencesToProto(declaration.Implements),
			References: typeReferencesToProto(declaration.References),
		})
	}
	return result
}

func incomingTypeRelationsToProto(
	relations []query.IncomingTypeRelation,
) []*goconductv1.IncomingTypeRelation {
	result := make([]*goconductv1.IncomingTypeRelation, 0, len(relations))
	for _, relation := range relations {
		result = append(result, &goconductv1.IncomingTypeRelation{
			Kind:            relation.Kind,
			SourceId:        relation.SourceID,
			SourceComponent: relation.SourceComponent,
...
func typeFieldsToProto(fields []TypeField) []*goconductv1.TypeField {
	result := make([]*goconductv1.TypeField, 0, len(fields))
	for _, field := range fields {
		result = append(result, &goconductv1.TypeField{
			Name:     field.Name,
			Type:     field.Type,
...
func typeMethodsToProto(methods []TypeMethod) []*goconductv1.TypeMethod {
	result := make([]*goconductv1.TypeMethod, 0, len(methods))
	for _, method := range methods {
		result = append(result, &goconductv1.TypeMethod{
			Name:            method.Name,
...
func typeReferencesToProto(references []TypeReference) []*goconductv1.TypeReference {
	result := make([]*goconductv1.TypeReference, 0, len(references))
	for _, reference := range references {
		result = append(result, &goconductv1.TypeReference{
