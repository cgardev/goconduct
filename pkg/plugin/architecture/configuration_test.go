...
func TestApplicationConfiguration_LoadDefaults(t *testing.T) {
	t.Run("Scenario: The optional configuration document is absent", func(t *testing.T) {
		...
		t.Run("And unmatched dependencies remain allowed for incremental adoption", func(t *testing.T) {
			dependencies := configuration.Architecture.Dependencies
			if dependencies.ProductionDefault != DependencyDefaultAllow ||
				dependencies.TestDefault != DependencyDefaultAllow {
				t.Errorf("unexpected dependency defaults: %+v", dependencies)
			}
		})
	})
}
