...
func TestFunctionAnalysis_InspectTypedPackage(t *testing.T) {
	t.Run("Scenario: One package contains functions, methods, initializers, and unresolved calls", func(
		...
		t.Run("When the analyzer inspects the loaded package twice", func(*testing.T) {
			...
			secondDeclarations, secondReferences, _ = sourceAnalyzer.inspectLoadedPackage(
				"example.com/functions",
				loadedPackage,
				fileSet,
				selectedPaths,
...
func TestFunctionAnalysis_RejectUnclassifiedCalls(t *testing.T) {
	t.Run("Scenario: Function objects and calls cross the configured analysis boundary", func(t *testing.T) {
		...
		t.Run("When the analyzer classifies each function object and call", func(*testing.T) {
			...
			workDeclaration, _, _ = sourceAnalyzer.functionDeclarationFromObject(
				"example.com/functions",
				workFunction,
				fileSet,
				selectedPaths,
			)
			_, outsideScopeClassified, _ := sourceAnalyzer.functionDeclarationFromObject(
				"example.com/functions",
				runFunction,
				fileSet,
				make(stringSet),
			)
			if outsideScopeClassified {
				outsideScopeDeclaration, _, _ = sourceAnalyzer.functionDeclarationFromObject(
					"example.com/functions",
					runFunction,
					fileSet,
					make(stringSet),
			...
			packageDeclaration, _, _ = sourceAnalyzer.functionDeclarationFromObject(
				"example.com/functions",
				packageFunction,
				fileSet,
				selectedPaths,
			...
			_, unclassifiedCalls, _ = sourceAnalyzer.callsInNode(
			...
			_, nestedUnresolvedCalls, _ = sourceAnalyzer.callsInNode(
				"example.com/functions",
				unresolvedPackage,
				token.NewFileSet(),
				unresolvedNode,
			...
			_, nestedExternalCalls, _ = sourceAnalyzer.callsInNode(
				"example.com/functions",
				externalCallPackage,
				token.NewFileSet(),
				externalNode,
...
func TestFunctionAnalysis_InspectInitializerBoundaries(t *testing.T) {
	t.Run("Scenario: Package variables can contain calls or plain values", func(t *testing.T) {
		...
		t.Run("When the analyzer inspects each initializer boundary", func(step *testing.T) {
			...
			plainDeclarations, plainCalls, _ = sourceAnalyzer.inspectPackageInitializers(
				"example.com/functions",
				loadedPackage,
				fileSet,
				plainVariable,
