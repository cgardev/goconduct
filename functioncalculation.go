...
func buildFunctions(functionData map[string]*functionAccumulator) ([]Function, [][]string) {
	...
	for _, identifier := range identifiers {
		...
		functions = append(functions, Function{
			...
			CrossComponentCalledFunctions: len(data.crossComponentCallees),
			TransitiveCallerFunctions:     len(reachable(identifier, productionCallers)),
			TransitiveCalledFunctions:     len(reachable(identifier, productionDependencies)),
...
func attachFunctionCalls(graph *Graph, components map[string]*Component) {
	...
	callingFunctions := make(map[relationshipKey]stringSet)
	calledFunctions := make(map[relationshipKey]stringSet)
	for index := range graph.Relationships {
		...
		callingFunctions[key] = make(stringSet)
		calledFunctions[key] = make(stringSet)
	}
	for _, call := range graph.FunctionCalls {
		...
		callingFunctions[key].add(call.Source)
		calledFunctions[key].add(call.Target)
	}
	for key, relationship := range relationships {
		relationship.CallingFunctions = len(callingFunctions[key])
		relationship.CalledFunctions = len(calledFunctions[key])
	}
}
