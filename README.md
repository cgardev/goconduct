...
- Independently usable evaluator packages under `pkg/plugin/<name>`.
...
- Physical Go line and declared function measurements at every source scope.
...
| `loc` | Measures files, lines, functions, tests, and generated sources. | None |
...
goconduct loc --repository . --path internal --exclude '**/testdata/**'
goconduct mutation --repository . --path pkg/plugin
```
...
The full `loc` report preserves every normalized metric for automation:

```sh
goconduct loc --repository .
```

Focused queries filter, sort, and limit LOC evidence before writing JSON:

```sh
goconduct loc summary --repository .
goconduct loc packages --repository . --sort handwritten --limit 20
goconduct loc files --repository . --kind handwritten --sort total --limit 20
goconduct loc functions --repository . --kind handwritten --sort code --limit 20
```

File and function queries select handwritten sources by default.
Use `--kind all`, `--kind test`, or `--kind generated` to select another source classification.
Every list result reports its total matches and returned values.
...
goconduct component pkg/plugin/architecture --cache local
```
...
### LOC source selection

The `loc` plugin applies shared glob patterns after resolving its source roots.
The `**` segment crosses directory boundaries, and exclusions take precedence.

Generated detection combines the standard Go marker, path patterns, and RE2
expressions over the source header. A handwritten override takes precedence.
  ...
  "quality": {
    "loc": {
      "selection": {
        ...
        "exclude": ["**/testdata/**", "internal/legacy/**"]
      },
      "generated": {
        "standardMarker": true,
        "pathPatterns": ["**/*.pb.go", "**/*.connect.go"],
        "headerPatterns": ["(?m)^// AUTOGENERATED$"],
        "forceHandwrittenPaths": ["internal/protogen/manual.pb.go"]
      }
    }
  }
}
...
Its production build is copied into `pkg/plugin/architecture/_resources/web` and embedded with `go:embed`.
...
pkg/failure/                          Public error categories for every package
...
pkg/plugin/                           Public plugin SDK and evidence model
pkg/plugin/architecture/              Architecture evaluator and embedded dashboard
pkg/plugin/coverage/                  Coverage evaluator
pkg/plugin/crap/                      CRAP evaluator
pkg/plugin/duplication/               Duplication evaluator
pkg/plugin/mutation/                  Mutation evaluator
pkg/policy/                           Public path-policy resolver
pkg/report/                           Public architecture report model
...
It runs the Go server from source analyzing this repository, plus the Angular
development server with live reload on <http://127.0.0.1:4200>.
Set `GOCONDUCT_ADDRESS` to move the Go server off the default `127.0.0.1:6062`.
